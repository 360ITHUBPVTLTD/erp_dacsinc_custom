...
def get_item_stock_details_bulk(item_bom_pairs, sales_order_name):
    ...
    # Whether this order is already locked to the Delivery Note route or the
    # direct-billing (Sales Invoice + Update Stock) route by an earlier line
    # — used client-side to pre-hide the blocked option in the "Waiting for
    # DN / SI" prompt rather than only failing at submit time (the real
    # enforcement is server-side, see guard_so_fulfillment_route_lock).
    so_route_lock = ""
    if so_doc:
        if _so_has_submitted_dn(so_doc.name):
            so_route_lock = "dn"
        elif _so_has_submitted_stock_si(so_doc.name):
            so_route_lock = "si"
    ...
    for pair in item_bom_pairs:
        ...
        # along with sales_order_item and bom for filtering.
        raw_completed_receipts = frappe.db.sql("""
            (SELECT
                pr.name AS pr_name,
                '' AS sr_name,
                pri.purchase_order AS po_name,
        ...
        raw_draft_receipts = frappe.db.sql("""
            (SELECT
                pr.name AS pr_name, '' AS sr_name,
                pri.purchase_order AS po_name, pr.posting_date, pri.qty,
                pri.sales_order_item, pri.bom,
        ...
        draft_receipt_docs = []
        for r in raw_draft_receipts:
            if r.sales_order_item:
                if r.sales_order_item in so_item_names:
                    draft_receipt_docs.append(r)
            else:
                if bom_no:
                    if r.bom == bom_no:
                        draft_receipt_docs.append(r)
        ...
        raw_picks = frappe.db.sql("""
            SELECT
                pl.name AS pick_list_name, pl.docstatus, pl.per_delivered,
                pl.delivery_status, pl.status AS pick_status,
                pl.customer, pl.customer_name,
                pli.name AS pick_list_item,
                pli.qty, pli.picked_qty, IFNULL(pli.delivered_qty, 0) AS delivered_qty,
                pli.stock_qty, pli.conversion_factor, pli.uom,
                pli.warehouse, pli.item_name,
                pli.stock_uom, pl.purpose, soi.bom_no
            FROM `tabPick List Item` pli
            JOIN `tabPick List` pl ON pli.parent = pl.name
        ...
        for p in raw_picks:
            if p.pick_status == "Completed":
                p.delivered_qty = p.picked_qty

        # The BOM filter exists only to separate two lines of the SAME item that use
        # DIFFERENT BOMs. Applied unconditionally it hid real Pick Lists whenever the
        # row's Sales Order Item link was missing, so a pick against the order showed
        # up as "Picked (This SO) = 0". Narrow by BOM only when the order is actually
        # ambiguous, and never drop a row we cannot classify.
        so_boms_for_item = {
            (i.bom_no or "").strip()
            for i in so_items_list
            if (i.item_code or "").strip() == item_code
        }
        want_bom = (bom_key if bom_key != 'no_bom' else "").strip()

        if len(so_boms_for_item) > 1:
            current_picks = [
                p for p in raw_picks
                if p.bom_no is None or (p.bom_no or "").strip() == want_bom
            ]
        else:
        ...
        total_hist_recv = sum(flt(r.received_qty) for r in completed_receipt_docs)
        recv_for_so_qty = min(max(0, total_hist_recv - delivered_qty), total_available_stock)
        general_stock_qty = max(0, total_available_stock - recv_for_so_qty - picked_submitted_other_rows - picked_draft_other_rows)
        ...
        # Row-level delivered_qty is the honest figure; per_delivered only moves when the
        # Delivery Note was raised FROM the Pick List.
        raw_sub_undelivered = sum(
            max(0, flt(p.picked_qty) - flt(p.delivered_qty))
            for p in current_picks if p.docstatus == 1
        )

        # Edge case: a Delivery Note raised directly off the Sales Order bumps
        # soi.delivered_qty but leaves the Pick List open and its delivered_qty at 0,
        # so delivered + picked would double-count. Cap the pipeline at what the line
        # still owes, and keep the difference as "stale" so the UI can flag it.
        outstanding_line_qty  = max(0, required_qty - delivered_qty)
        picked_sub_undelivered = min(raw_sub_undelivered, outstanding_line_qty)
        picked_draft_qty_so   = min(picked_draft_qty_raw,
                                    max(0, outstanding_line_qty - picked_sub_undelivered))
        stale_pick_qty        = (raw_sub_undelivered - picked_sub_undelivered) \
                                + (picked_draft_qty_raw - picked_draft_qty_so)

        # Pick Lists still open on a line that has already been delivered elsewhere.
        stale_pick_lists = []
        if stale_pick_qty > 0:
            stale_pick_lists = [
                {"pick_list_name": p.pick_list_name, "docstatus": p.docstatus,
                 "qty": flt(p.qty), "picked_qty": flt(p.picked_qty),
                 "delivered_qty": flt(p.delivered_qty), "pick_status": p.pick_status}
                for p in current_picks
                if (p.docstatus == 0 and flt(p.qty) > 0)
                or (p.docstatus == 1 and max(0, flt(p.picked_qty) - flt(p.delivered_qty)) > 0)
            ]

        # 6. Conflicts (Picks for OTHER orders)
        conflict_details = []
        if sales_order_name:
            # A pick only conflicts while it is still HOLDING stock. Once the other
            # order has been delivered — whether through the Pick List or by a
            # Delivery Note raised straight off that Sales Order — the stock is gone
            # and the row must not be reported as a live conflict.
            conflict_sql = """
                SELECT * FROM (
                    SELECT pl.name AS pick_list_name, pl.customer, pl.customer_name,
                           pli.sales_order, pli.sales_order_item AS so_item,
                           so.customer AS so_customer, so.customer_name AS so_customer_name,
                           so.transaction_date AS so_date, so.delivery_date AS so_delivery_date,
                           so.status AS so_status,
                           pl.docstatus, CASE WHEN pl.docstatus=1 THEN 'Submitted' ELSE 'Draft' END AS status,
                           pl.status AS pick_status, pli.qty, pli.picked_qty,
                           IFNULL(pli.delivered_qty, 0) AS delivered_qty,
                           pli.warehouse, pli.item_name,
                           LEAST(
                               CASE WHEN pl.docstatus = 1
                                    THEN GREATEST(IFNULL(pli.picked_qty, 0) - IFNULL(pli.delivered_qty, 0), 0)
                                    ELSE IFNULL(pli.qty, 0) END,
                               CASE WHEN soi.name IS NULL THEN 999999999
                                    ELSE GREATEST(IFNULL(soi.qty, 0) - IFNULL(soi.delivered_qty, 0), 0) END
                           ) AS held_qty
                    FROM `tabPick List Item` pli
        ...
        picked_for_others_qty = sum(flt(r.held_qty) for r in conflict_details if r.docstatus == 1)
        draft_qty_for_others = sum(flt(r.held_qty) for r in conflict_details if r.docstatus == 0)

        # 7. Incoming Pipeline (Current SO POs/EWOs)
        # ------------- MODIFICATION STARTS HERE (Current SO POs) ------------------
        # Matching by sales_order + item_code alone conflates two different SO
        # lines that share the same item_code but differ by bom_no (one with a
        # BOM meant for subcontract, one without meant for plain purchase) —
        # a PO raised against only one of those lines would show as "incoming"
        ...
        raw_po_data = frappe.db.sql(f"""
            SELECT
                ...
                poi.sales_order_item, poi.bom,
                COALESCE(poi.expected_delivery_date, poi.schedule_date, po.schedule_date) AS expected_delivery_date,
                CASE WHEN po.is_subcontracted = 1 THEN (poi.fg_item_qty - poi.received_qty) ELSE (poi.qty - poi.received_qty) END AS pending_qty,
                CASE WHEN po.is_subcontracted = 1 THEN poi.fg_item_qty ELSE poi.qty END AS ordered_qty,
        ...
        filtered_po_items = []
        for r in raw_po_data:
            if r.sales_order_item:
                if r.sales_order_item in so_item_names:
                    filtered_po_items.append(r)
            else:
                if bom_no:
                    if r.bom == bom_no:
                        filtered_po_items.append(r)
        ...
        unique_pos = {}
        for r in filtered_po_items:
            qty = flt(r.pending_qty)
            if qty <= 0: continue

            entry = unique_pos.setdefault(r.name, {
                "doc_type": "Purchase Order", "name": r.name, "info": r.supplier_name or r.supplier,
                "supplier": r.supplier, "supplier_name": r.supplier_name,
                "item_name": r.item_name, "status": r.po_status, "date": r.po_date,
                "expected_delivery_date": r.expected_delivery_date,
                "warehouse": r.warehouse, "is_ewo": 0, "is_subcontracted": r.is_subcontracted,
                "pending_qty": 0.0, "ordered_qty": 0.0, "received_qty": 0.0
            })
            entry["pending_qty"] += qty
            entry["ordered_qty"] += flt(r.ordered_qty)
            entry["received_qty"] += flt(r.received_qty)

        for entry in unique_pos.values():
            incoming_stock.append(entry)
            total_incoming_qty += entry["pending_qty"]
            total_incoming_po_count += 1
        ...
        for r in raw_draft_po_data:
            if r.sales_order_item:
                if r.sales_order_item in so_item_names:
                    filtered_draft_po_items.append(r)
            else:
                if bom_no:
                    if r.bom == bom_no:
                        filtered_draft_po_items.append(r)
        ...
        unique_draft_pos = {}
        for r in filtered_draft_po_items:
            qty = flt(r.qty)
            if qty <= 0: continue
            
            entry = unique_draft_pos.setdefault(r.name, {
                "name": r.name, "info": r.supplier_name or r.supplier,
                "supplier": r.supplier, "supplier_name": r.supplier_name,
                "date": r.po_date, "expected_delivery_date": r.expected_delivery_date,
                "warehouse": r.warehouse,
                "qty": 0.0, "is_subcontracted": r.is_subcontracted,
            })
            entry["qty"] += qty

        draft_purchase_orders = list(unique_draft_pos.values())

        # 7b. Embroidery Work Orders (EWOs)
        # Fetch EWO items matching the item code and the linked Purchase Order Item,
        # filtering in Python using the same SO Item / BOM matching logic.
        raw_ewo_data = frappe.db.sql("""
             SELECT
                parent.name, parent.date, parent.purchase_order, parent.work_type,
                parent.panel_jobber, parent.panel_stage, parent.full_piece_jobber, parent.full_piece_stage,
                COALESCE(fp_sup.supplier_name, panel_sup.supplier_name) AS jobber_name,
                parent.status AS ewo_status, parent.completed_on,
                poi.sales_order_item, poi.bom AS po_bom,
                child.item_name AS item_name,
                child.ordered_qty AS ordered_qty,
                child.received_qty AS received_qty,
                IFNULL(child.pending_qty, 0) AS pending_qty
            FROM `tabEmbroidery Work Order Item` child
            JOIN `tabEmbroidery Work Order` parent ON child.parent = parent.name
            LEFT JOIN `tabSupplier` fp_sup ON parent.full_piece_jobber = fp_sup.name
            LEFT JOIN `tabSupplier` panel_sup ON parent.panel_jobber = panel_sup.name
        ...
        filtered_ewos = []
        for r in raw_ewo_data:
            if r.sales_order_item:
                if r.sales_order_item in so_item_names:
                    filtered_ewos.append(r)
            else:
                if bom_no:
                    if r.po_bom == bom_no:
                        filtered_ewos.append(r)
        ...
        for row in filtered_ewos:
            ...
            entry = unique_ewos.setdefault(row.name, {
                "doc_type": "Embroidery Work Order", "name": row.name, "date": row.date,
                "work_type": row.work_type, "info": info_text, "jobber_id": jobber_id,
                "jobber_name": row.jobber_name, "item_name": row.item_name,
                "status": row.ewo_status, "stage": stage,
                "expected_delivery_date": row.completed_on or row.date,
                "po_ref": row.purchase_order, "pending_qty": 0.0,
                "ordered_qty": flt(row.ordered_qty), "received_qty": flt(row.received_qty),
                "is_ewo": 1
            })
            entry["pending_qty"] += qty

        for entry in unique_ewos.values():
            incoming_stock.append(entry)
            total_incoming_qty += entry["pending_qty"]
        total_incoming_ewo_count = len(unique_ewos)
        ...
        # An MR is a request, not stock in transit, so it is tracked separately from
        # incoming_stock. "Pending" is qty - ordered_qty: the part not yet turned into
        # a Purchase Order. Once a PO is raised, ordered_qty rises and this falls, so
        # the MR and the PO can never both count the same units.
        mr_data = frappe.db.sql("""
            SELECT
                mr.name, mr.material_request_type, mr.status, mr.transaction_date,
                mr.per_ordered, mr.per_received,
                mri.name AS mr_item, mri.qty, mri.ordered_qty, mri.received_qty,
                mri.stock_qty, mri.uom, mri.stock_uom, mri.warehouse, mri.schedule_date,
                GREATEST(IFNULL(mri.qty, 0) - IFNULL(mri.ordered_qty, 0), 0) AS pending_qty,
                GREATEST(IFNULL(mri.qty, 0) - IFNULL(mri.received_qty, 0), 0) AS awaiting_receipt_qty
            FROM `tabMaterial Request Item` mri
            JOIN `tabMaterial Request` mr ON mr.name = mri.parent
            WHERE mri.item_code = %(item)s
              AND mri.sales_order = %(so)s
              AND mr.docstatus = 1
        ...
        material_requests = []
        total_mr_qty = 0
        total_mr_pending_qty = 0
        for row in mr_data:
            total_mr_qty += flt(row.qty)
            total_mr_pending_qty += flt(row.pending_qty)
            material_requests.append({
                "name": row.name, "mr_item": row.mr_item,
                "material_request_type": row.material_request_type,
                "status": row.status, "date": row.transaction_date,
                "schedule_date": row.schedule_date,
                "qty": flt(row.qty), "ordered_qty": flt(row.ordered_qty),
                "received_qty": flt(row.received_qty),
                "pending_qty": flt(row.pending_qty),
                "awaiting_receipt_qty": flt(row.awaiting_receipt_qty),
        ...
        if item_meta.get("is_sub_contracted_item") or bom_no:
            sc_defaults = frappe.db.sql("""
                SELECT poi.item_code AS service_item, po.supplier, sup.supplier_name
                FROM `tabPurchase Order Item` poi
                JOIN `tabPurchase Order` po ON po.name = poi.parent
                LEFT JOIN `tabSupplier` sup ON sup.name = po.supplier
                WHERE po.is_subcontracted = 1 AND poi.fg_item = %s AND po.docstatus < 2
                ORDER BY po.creation DESC LIMIT 1
            """, item_code, as_dict=1)
            sc_defaults = sc_defaults[0] if sc_defaults else {}

            if not sc_defaults.get("service_item"):
                # No history for this item — fall back to the service item most
                # recently used on any subcontracted PO.
                last_service = frappe.db.sql("""
                    SELECT poi.item_code AS service_item
                    FROM `tabPurchase Order Item` poi
                    JOIN `tabPurchase Order` po ON po.name = poi.parent
                ...
                """, as_dict=1)
                if last_service:
                    sc_defaults["service_item"] = last_service[0].service_item
        ...
        stale_blockers = [
            ...
            for b in blockers if flt(b.so_delivered) >= flt(b.qty) > 0
        ]
        blocked_qty = sum(flt(b.held_qty) for b in blockers)
        ...
        rm_procurement_status = {
            ...
            # True when at least one RM's coverage comes from material already
            # SENT to the jobber (rm_transferred_to_sc_total) — a materially
            # different state from "Requested": that raw material has already
            # left the shelf and job work has physically started on however
            # much of it went. Calling that "Requested — Awaiting Arrival" (the
        ...
        if bom_no:
            try:
                ...
                if rm_codes:
                    ...
                    # yet, but shows a freshly-raised draft rather than nothing.
                    draft_po_data = frappe.db.sql("""
                        SELECT poi.item_code, GROUP_CONCAT(DISTINCT poi.parent) as po_list
                        FROM `tabPurchase Order Item` poi
                        JOIN `tabPurchase Order` po ON poi.parent = po.name
                        WHERE poi.sales_order = %(so)s AND poi.item_code IN %(items)s AND po.docstatus = 0
                        GROUP BY poi.item_code
                    ...
                    for d in draft_po_data:
                        rm = rm_map.get(d.item_code)
                        if rm:
                            rm["draft_po_documents"] = list(set(
                                rm.get("draft_po_documents", []) + (d.po_list.split(",") if d.po_list else [])
                            ))

                    # 4. Raw material already sent to a subcontractor for THIS SO
                    # AND still sitting there unconsumed. Once material physically
                    # leaves VV Puram for the jobber, it no longer shows up in
                    # rm_available_stock there — but it hasn't been wasted, it's
                    # been committed to producing this exact order, so it must
                    # still count as coverage RIGHT UP UNTIL a Subcontracting
                    # Receipt actually consumes it. Once consumed, it has already
                    # turned into finished-good stock (which is what shrinks
                    # fg_shortfall itself) — counting it here TOO would be
                    # crediting the same material twice: once as reduced
                    # fg_shortfall, again as RM coverage for whatever NEW
                    # production is still needed beyond that. Subcontracting
                    # Order Supplied Item is ERPNext's own running ledger of
                    # this — supplied_qty minus consumed_qty minus returned_qty
                    # is exactly what's still outstanding at the jobber, whereas
                    # summing Stock Entry Detail (the raw transfer amount, ever
                    # sent) doesn't know a Subcontracting Receipt happened since.
                    #
                    # A subcontracted PO links back to a Sales Order on its own
                    # service-item row (Purchase Order Item.sales_order) — there
                    # is no `Purchase Order Raw Material Source` row for these
                    # (that child table is only populated by the separate
                    # plain-purchase RM flow). And a single subcontracted PO can
                    # combine service-item rows for SEVERAL different Sales
                    # Orders against the same finished good (each its own row,
                    # its own fg_item_qty), all sharing ONE Subcontracting Order
                    # — the outstanding-supplied figure has no per-SO breakdown.
                    # So this SO's credit is its own fg_item_qty's share of the
                    # PO's total fg_item_qty for this finished good, applied to
                    # that outstanding quantity — both subqueries pre-aggregate
                    # to one row per PO first, so a PO with more than one row for
                    # the same SO+finished-good is never fanned out and
                    # double-counted.
                    transfer_data = frappe.db.sql("""
                        SELECT
                            sosi.rm_item_code as item_code,
                            SUM(
                                (sosi.supplied_qty - sosi.consumed_qty - sosi.returned_qty)
                                * this_so.so_fg_qty / po_fg_totals.total_fg_qty
                            ) as outstanding_qty,
                            GROUP_CONCAT(DISTINCT sosi.parent) as sco_list,
                            GROUP_CONCAT(DISTINCT sco.purchase_order) as po_list
                        ...
                        JOIN (
                            SELECT parent, SUM(fg_item_qty) as so_fg_qty
                            FROM `tabPurchase Order Item`
                            WHERE sales_order = %(so)s AND fg_item = %(fg_item)s AND fg_item_qty > 0
                            GROUP BY parent
                        ) this_so ON this_so.parent = sco.purchase_order
                        JOIN (
                            SELECT parent, SUM(fg_item_qty) as total_fg_qty
                            FROM `tabPurchase Order Item`
                            WHERE fg_item = %(fg_item)s AND fg_item_qty > 0
                              AND sales_order IS NOT NULL AND sales_order != ''
                            GROUP BY parent
                        ) po_fg_totals ON po_fg_totals.parent = sco.purchase_order
                        WHERE sosi.rm_item_code IN %(items)s
                        GROUP BY sosi.rm_item_code
                    """, {"so": sales_order_name, "fg_item": item_code, "items": tuple(rm_codes)}, as_dict=1)

                    for d in transfer_data:
                        rm = rm_map.get(d.item_code)
                        if rm:
                            transferred_qty = max(0, flt(d.outstanding_qty, 2))
                            rm["rm_transferred_to_sc_total"] = transferred_qty
                            if transferred_qty > 0:
                                rm["transfer_documents"] = d.sco_list.split(",") if d.sco_list else []
                                # The user manages this whole flow from the Purchase
                                # Order, not the Subcontracting Order directly — a
                                # reference naming only the SCO leaves them with no
                                # way back to the document they actually work from.
                                rm["transfer_po_documents"] = d.po_list.split(",") if d.po_list else []

                    # 5. Where the rest of a bulk RM purchase actually went — a raw
                    # material bought in one large batch (one PO covering many
                    # orders' worth) is a SHARED pool: once part of it has been
                    # sent to a subcontractor for a DIFFERENT Sales Order and NOT
                    # YET consumed there, it's gone from that shelf same as if it
                    # were sent for THIS one, and "Stock: 13.00" alone doesn't
                    # explain why that's lower than the full purchase would
                    # suggest. Same outstanding-only rule as this SO's own share
                    # above (supplied minus consumed minus returned) — once a
                    # Subcontracting Receipt consumes it, it has already turned
                    # into THAT order's finished-good stock and stopped being
                    # "elsewhere with your raw material", it's just a completed,
                    # unrelated transaction.
                    total_transfer_data = frappe.db.sql("""
                        ...
                        WHERE sosi.rm_item_code IN %(items)s
                        GROUP BY sosi.rm_item_code
                    """, {"items": tuple(rm_codes)}, as_dict=1)
                    for d in total_transfer_data:
                        rm = rm_map.get(d.item_code)
                        if rm:
                            rm["rm_transferred_to_other_so_total"] = max(
                ...
                for rm in rm_map.values():
                    ...
                    if needed_r <= 0:
                        ...
                        # jobber — job work has physically started on that
                        # much, a materially further-along state than a plain
                        # pending MR/PO nothing has moved on yet. Still true
                        # even when it's a MIX (partly at the jobber, partly
                        # still just a pending MR/PO) — the client renders the
                        # actual split (see rm_transferred_to_sc_total vs
                        # rm_pending_mr_total/rm_pending_so_linked_total on
                        # this same row) rather than collapsing it to one word.
                        rm["status"] = "In Process at Jobber"
                    else:
                        rm["status"] = "Requested"
                    if shortfall > 0:
                        rm_procurement_status["rm_shortfall_exists"] = True
        ...
        results[pair] = {
            ...
            "picked_submitted_qty_so_actual": picked_sub_qty_so_actual,
            "picked_submitted_undelivered_qty": picked_sub_undelivered,
            "stale_pick_qty": stale_pick_qty, "stale_pick_lists": stale_pick_lists,
            "blocked_qty": blocked_qty, "stale_blocked_qty": stale_blocked_qty,
            ...
            "subcontract_supplier": sc_defaults.get("supplier"),
            "subcontract_supplier_name": sc_defaults.get("supplier_name"),
            "rm_procurement_status": rm_procurement_status,
            "customer": so_doc.customer if so_doc else None,
...
def get_linked_delivery_notes(sales_order_name):
    ...
    return frappe.db.get_all(
        ...
        fields=["name", "customer", "customer_name", "posting_date", "status",
                "grand_total", "currency", "per_billed", "lr_no"],
...
def _get_pick_blockers(item_codes):
    """
    Pick Lists that ERPNext still counts as HOLDING stock of these items.

    Mirrors PickList._get_pick_list_items(): anything not Completed/Cancelled keeps
    its picked (or planned) quantity reserved, which is subtracted from Bin stock
    when locations are worked out. A Pick List left Open after the goods were
    delivered straight off the Sales Order therefore blocks all further picking.

    `so_delivered` is read from Delivery Note Item.so_detail, which is populated
    whether or not the Delivery Note came from the Pick List — that is how a stale
    row is detected.
    """
    if not item_codes:
        return []
    ...
    return frappe.db.sql("""
        SELECT
            pl.name AS pick_list, pl.status AS pick_status, pl.per_delivered,
            pli.item_code, pli.warehouse, pli.sales_order, pli.sales_order_item,
            pli.qty, pli.picked_qty, IFNULL(pli.delivered_qty, 0) AS delivered_qty,
            CASE WHEN pli.picked_qty > 0 AND pli.docstatus = 1
                 THEN pli.picked_qty - IFNULL(pli.delivered_qty, 0)
                 ELSE pli.stock_qty END AS held_qty,
            ((SELECT IFNULL(SUM(dni.qty), 0) FROM `tabDelivery Note Item` dni
               WHERE dni.so_detail = pli.sales_order_item AND dni.docstatus = 1)
             +
             (SELECT IFNULL(SUM(sii.qty), 0) FROM `tabSales Invoice Item` sii
               JOIN `tabSales Invoice` si ON si.name = sii.parent
               WHERE sii.so_detail = pli.sales_order_item AND si.docstatus = 1 AND IFNULL(si.update_stock, 0) = 1)
            ) AS so_delivered
        FROM `tabPick List Item` pli
        JOIN `tabPick List` pl ON pl.name = pli.parent
...
@frappe.whitelist()
def reconcile_pick_lists(pick_lists=None, sales_order=None, item_code=None):
    """
    (Called from the Sales Order stock widget → "Fix stale Pick Lists")

    Re-runs the delivery calculation on submitted Pick Lists and writes the result.
    A Pick List whose Sales Order line was delivered by another route keeps
    per_delivered = 0 and status "Open" forever, and while it is Open ERPNext keeps
    its quantity reserved — which silently blocks every future pick of that item.

    This is the same computation the Delivery Note hook performs; running it on
    demand repairs rows whose Delivery Note was submitted before the hook existed
    or whose hook run failed.
    """
    ...
    names = list(pick_lists or [])

    if not names and (sales_order or item_code):
        filters = {"docstatus": 1}
        if sales_order:
            filters["sales_order"] = sales_order
        if item_code:
            filters["item_code"] = item_code
        names = frappe.db.get_all("Pick List Item", filters=filters, pluck="parent", distinct=True)

    if not names:
        return {"updated": [], "unchanged": [], "message": _("No Pick Lists to reconcile.")}
    ...
    for name in names:
        try:
            ...
            new_status = _calculate_pick_list_delivery_status(pl_doc)
            changed = (
                flt(pl_doc.per_delivered) != flt(new_status.get("per_delivered"))
                or pl_doc.status != new_status.get("status")
                or pl_doc.get("delivery_status") != new_status.get("delivery_status")
            )
            if changed:
                frappe.db.set_value("Pick List", name, new_status)
                updated.append({"pick_list": name, **new_status})
            else:
                unchanged.append(name)
        except Exception:
            frappe.log_error(title=f"Reconcile failed for Pick List {name}",
                             message=frappe.get_traceback())

    return {
        "updated": updated,
        "unchanged": unchanged,
        "message": _("{0} Pick List(s) updated.").format(len(updated)) if updated
                   else _("Nothing needed changing.")
    }


@frappe.whitelist()
def get_linked_procurement_docs(sales_order_name):
    """
    (Called from the Sales Order stock widget)

    The buying side of a Sales Order in one place: what was requested (Material
    Request), what was ordered (Purchase Order) and what actually arrived
    (Purchase Receipt / Subcontracting Receipt).

    Quantities are aggregated over the rows that belong to THIS Sales Order, so a
    Purchase Order covering several orders reports only this order's share.
    """
    if not sales_order_name:
        return {"material_requests": [], "purchase_orders": [], "purchase_receipts": []}

    material_requests = frappe.db.sql("""
        SELECT
            mr.name, mr.transaction_date, mr.schedule_date, mr.material_request_type,
        ...
        FROM `tabMaterial Request Item` mri
        JOIN `tabMaterial Request` mr ON mr.name = mri.parent
        WHERE mri.sales_order = %(so)s AND mr.docstatus < 2
        GROUP BY mr.name, mr.transaction_date, mr.schedule_date, mr.material_request_type,
                 mr.status, mr.docstatus, mr.per_ordered, mr.per_received
        ORDER BY mr.transaction_date DESC
    """, {"so": sales_order_name}, as_dict=1)

    purchase_orders = frappe.db.sql("""
        SELECT
            po.name, po.transaction_date, po.schedule_date, po.status, po.docstatus,
            po.supplier, sup.supplier_name, po.is_subcontracted,
            po.currency, po.per_received, po.per_billed,
        ...
        GROUP BY po.name, po.transaction_date, po.schedule_date, po.status, po.docstatus,
                 ...
                 po.per_received, po.per_billed
        ORDER BY po.transaction_date DESC
    """, {"so": sales_order_name}, as_dict=1)

    # Purchase Receipts link to the order directly; Subcontracting Receipts only
    # through the Purchase Order Item they were raised against.
    purchase_receipts = frappe.db.sql("""
        (SELECT
            'Purchase Receipt' AS doctype, pr.name, pr.posting_date, pr.status, pr.docstatus,
            pr.supplier, sup.supplier_name, pr.is_subcontracted, pr.currency,
            COUNT(DISTINCT pri.item_code) AS item_count,
            SUM(pri.received_qty) AS qty, SUM(pri.amount) AS amount
         FROM `tabPurchase Receipt Item` pri
         JOIN `tabPurchase Receipt` pr ON pr.name = pri.parent
         LEFT JOIN `tabSupplier` sup ON sup.name = pr.supplier
         WHERE pri.sales_order = %(so)s AND pr.docstatus < 2
         GROUP BY pr.name, pr.posting_date, pr.status, pr.docstatus, pr.supplier,
                  sup.supplier_name, pr.is_subcontracted, pr.currency)

        UNION ALL

        (SELECT
            'Subcontracting Receipt' AS doctype, scr.name, scr.posting_date, scr.status, scr.docstatus,
            scr.supplier, sup.supplier_name, 1 AS is_subcontracted,
            NULL AS currency,   -- Subcontracting Receipt has no currency field
            COUNT(DISTINCT scri.item_code) AS item_count,
            SUM(scri.qty) AS qty, SUM(scri.amount) AS amount
         FROM `tabSubcontracting Receipt Item` scri
         ...
         WHERE poi.sales_order = %(so)s AND scr.docstatus < 2
         GROUP BY scr.name, scr.posting_date, scr.status, scr.docstatus, scr.supplier,
                  sup.supplier_name)
...
@frappe.whitelist()
def create_pick_list_for_items(sales_order, items=None):
    """
    (Called from the Sales Order stock widget)

    ERPNext only ships a whole-order mapper
    (erpnext.selling.doctype.sales_order.sales_order.create_pick_list), which maps
    every pending line. This reuses that mapper for correct field mapping and
    then trims the result to the requested rows and quantities.

    Always one Pick List per Sales Order line — never combines more than one
    line into a single Pick List, matching sales_order_on_submit's automatic
    behaviour (see _create_pick_lists_one_per_item). The per-row "Pick"
    button only ever requests one line, so that case still returns an
    UNSAVED Pick List for the user to review and save themselves. The bulk
    "Create Pick List" button can request several lines at once — those are
    inserted immediately, one Pick List per line, and their names are
    returned under "created" for the caller to open the review modal on
    (see get_pick_lists_for_so / show_so_picklists_modal in sales_order.js).

    items: [{"sales_order_item": "<SO Item row>", "item_code": "...", "qty": n}, ...]
           Pass nothing to keep every pending line.
    """
    from erpnext.selling.doctype.sales_order.sales_order import (
        create_pick_list as erpnext_create_pick_list,
    )

    if isinstance(items, str):
        items = json.loads(items or "[]")

    doc = erpnext_create_pick_list(sales_order)

    if not doc.get("locations"):
        # "Nothing pending" is rarely the real reason. The usual cause is another
        # Pick List still holding the stock — explain which one, and whether it is
        # stale, instead of leaving the user guessing.
        wanted_items = [r.get("item_code") for r in (items or []) if r.get("item_code")]
        if not wanted_items:
            wanted_items = list({i.item_code for i in frappe.get_doc("Sales Order", sales_order).items})

        blockers = _get_pick_blockers(wanted_items)
        blocking = [b for b in blockers if flt(b.held_qty) > 0]

        if blocking:
            stale = [b for b in blocking if flt(b.so_delivered) >= flt(b.qty) > 0]
            lines = []
            for b in blocking:
                tag = _(" — already delivered on {0}, so this Pick List is stale").format(b.sales_order) \
                      if b in stale else ""
                lines.append("• {0} ({1}) is holding {2} of {3}{4}".format(
                    b.pick_list, b.pick_status, flt(b.held_qty), b.item_code, tag))

            msg = _("Cannot build a Pick List — the stock is held by other Pick Lists:") \
                + "<br><br>" + "<br>".join(lines)

            if stale:
                msg += "<br><br>" + _(
                    "Those Sales Orders were delivered without going through the Pick List, "
                    "so it never closed. Use <b>Fix stale Pick Lists</b> on the stock table to "
                    "release the reservation, then create the Pick List again."
                )
            frappe.throw(msg, title=_("Stock is reserved elsewhere"))

        frappe.throw(_(
            "Nothing is pending to pick on {0} — every line is already delivered or fully picked."
        ).format(sales_order))

    if items:
        wanted_by_row, wanted_by_item = {}, {}
        for r in items:
            qty = flt(r.get("qty")) or None
            if r.get("sales_order_item"):
                wanted_by_row[r["sales_order_item"]] = qty
            if r.get("item_code"):
                wanted_by_item[r["item_code"]] = qty

        keep = []
        for loc in doc.locations:
            if loc.sales_order_item in wanted_by_row:
                new_qty = wanted_by_row[loc.sales_order_item]
            elif loc.item_code in wanted_by_item:
                new_qty = wanted_by_item[loc.item_code]
            else:
                continue

            # Never pick more than the mapper says is still outstanding.
            if new_qty and new_qty < flt(loc.qty):
                loc.qty = new_qty
                loc.stock_qty = new_qty * flt(loc.conversion_factor or 1)
                if loc.get("picked_qty"):
                    loc.picked_qty = 0
            keep.append(loc)

        if not keep:
            frappe.throw(_(
                "Nothing pending to pick for the selected item on {0}. "
                "It may already be delivered or fully picked."
            ).format(sales_order))

        doc.locations = keep

    distinct_lines = {loc.sales_order_item or loc.item_code for loc in doc.locations}

    if len(distinct_lines) <= 1:
        for idx, loc in enumerate(doc.locations, start=1):
            loc.idx = idx
        doc.insert(ignore_permissions=True)
        return {"created": [doc.name]}

    created = _split_and_insert_pick_lists(doc)
    if not created:
        frappe.throw(_("Could not create any Pick List for {0} — see the Error Log.").format(sales_order))
...
def make_subcontract_purchase_order(sales_order, item_code, qty=None):
    ...
    try:
        ...
        sc_bom = get_subcontracting_boms_for_finished_goods([item_code]).get(item_code)
        if sc_bom:
            service_item = sc_bom.get("service_item")
    except Exception:
        frappe.log_error(title=f"Subcontracting BOM lookup failed for {item_code}",
                          message=frappe.get_traceback())

    defaults = frappe.db.sql("""
        SELECT poi.item_code AS service_item, po.supplier
        FROM `tabPurchase Order Item` poi
        JOIN `tabPurchase Order` po ON po.name = poi.parent
        LEFT JOIN `tabSupplier` sup ON sup.name = po.supplier
        WHERE po.is_subcontracted = 1 AND poi.fg_item = %s AND po.docstatus < 2
    ...
    if defaults:
        service_item = service_item or defaults[0].service_item
        supplier = defaults[0].supplier
    ...
    if not service_item:
        fallback = frappe.db.sql("""
            ...
            ORDER BY po.creation DESC LIMIT 1
        """, as_dict=1)
        service_item = fallback[0].service_item if fallback else None

    if not service_item:
        frappe.throw(_(
            "No subcontracting service item is configured for {0}. Add a Subcontracting "
            "BOM mapping this finished good to a service item, or create one subcontracted "
...
def _log_pick_list_qty_reduction(pick_list_name, reductions):
    """
    Leaves a visible, timestamped record whenever a Pick List line was
    allocated for more than what actually got picked (e.g. created for 5,
    only 2 picked and submitted) — on the Pick List's own timeline, and on
    every Sales Order it touched so it also surfaces in the Order Flow
    dashboard's activity stream, which only reads comments on Sales Order.
    Never allowed to block the actual save/submit if logging itself fails.
    """
    if not reductions:
        return
    try:
        pl_lines = "<br>".join(
            _("{0}: reduced from {1} to {2} — {3} left unpicked on this line.").format(
                r["item_code"], r["original_qty"], r["new_qty"],
                flt(r["original_qty"]) - flt(r["new_qty"])
            ) for r in reductions
        )
        frappe.get_doc("Pick List", pick_list_name).add_comment("Info", pl_lines)

        by_so = {}
        for r in reductions:
            if r.get("sales_order"):
                by_so.setdefault(r["sales_order"], []).append(r)

        for so_name, so_reductions in by_so.items():
            so_lines = "<br>".join(
                _("Pick List {0} — {1}: reduced from {2} to {3} ({4} left unpicked, still needs picking).").format(
                    pick_list_name, r["item_code"], r["original_qty"], r["new_qty"],
                    flt(r["original_qty"]) - flt(r["new_qty"])
                ) for r in so_reductions
            )
            frappe.get_doc("Sales Order", so_name).add_comment("Info", so_lines)
    except Exception:
        frappe.log_error(title=f"Could not log Pick List qty reduction for {pick_list_name}",
                          message=frappe.get_traceback())
...
def update_and_submit_pick_list(pick_list, rows=None, submit=1):
    ...
    if doc.docstatus != 0:
        frappe.throw(_("Pick List {0} is no longer a draft.").format(pick_list))

    doc.check_permission("write")

    requested = {}
    for r in rows:
        row_name = r.get("pick_list_item")
        if row_name:
            requested[row_name] = flt(r.get("qty"))

    changed = 0
    reductions = []  # audit trail: every line whose qty was actually reduced
    for loc in doc.locations:
        if loc.name not in requested:
            continue

        new_qty = requested[loc.name]
        if new_qty <= 0:
            frappe.throw(_("Quantity for {0} must be greater than zero.").format(loc.item_code))

        original_qty = flt(loc.qty)
        if new_qty > original_qty:
            frappe.throw(_(
                "Quantity for {0} cannot be increased to {1} — {2} was already picked/"
    ...
    if changed:
        doc.save()
        _log_pick_list_qty_reduction(doc.name, reductions)
    ...
    return {
        ...
        "changed": changed,
        "message": _("Pick List {0} updated.").format(doc.name)
    }
...
def _calculate_pick_list_delivery_status(pl_doc, allow_downgrade_from_completed=False):
    ...
    # Both fields are plain Select fields with a fixed options list — Pick
    # List.status is core ERPNext's own (Draft / Open / Partly Delivered /
    # Completed / Cancelled) and delivery_status is this app's own (Not
    # Delivered / Partly Delivered / Fully Delivered). Neither has a
    # "Submitted" or "Partially Delivered" option; writing those strings
    # here silently "succeeds" only because _reconcile_pick_lists_for_
    # sales_orders applies this via frappe.db.set_value (a raw DB write
    # that skips Select-option validation), leaving every reader of these
    # fields checking for values that were never actually valid.
    delivery_status = "Not Delivered"
    new_doc_status = "Open" # Nothing delivered yet is ERPNext's own post-submit default for this doctype
    ...
    if per_delivered > 0 and per_delivered < 100:
        ...
        new_doc_status = "Partly Delivered"
...
def update_pick_lists_on_stock_si_submit(doc, method):
    ...
    Invoice submitted with Update Stock — the other route that can deliver a
    Sales Order line without ever raising a Delivery Note.
    """
...
# Fields where a Pick List's own reservation is the one source of truth for
# a row built from it — a Delivery Note Item's own pick_list_item (set only
# by our create_dn_or_si_from_pick_lists / the standard Pick-List-to-DN
# mapper) means that exact qty/item/warehouse is what the Pick List already
# physically reserved. Sales Invoice Item has no such reference field, but
# in this company's flow the ONLY way to reach an Update Stock invoice
# against a Sales Order is the same create_dn_or_si_from_pick_lists action
# (guard_so_fulfillment_route_lock already establishes an order can't mix
# routes), so update_stock + so_detail is the equivalent signal there.
...
def _revert_pick_list_locked_fields(doc, is_locked_row):
    """
    Backs the client-side read_only_depends_on lock (public/js/delivery_note.js
    / sales_invoice.js) with a real guarantee — a client-side read-only can be
    bypassed via the API or a disabled script, and this is exactly the kind of
    mismatch (a DN/SI row silently ending up different from what its Pick List
    actually reserved) that quietly breaks picked_qty vs delivered_qty
    reconciliation everywhere the Order Flow dashboard depends on it. Only
    matters for a row that already existed on a PRIOR save — freshly mapped
    row being saved for the first time has no earlier value to protect, and
    IS the value the Pick List's reservation should take on.
    """
    if doc.is_new():
        return
    ...
    if not before:
        return
    before_by_name = {d.name: d for d in (before.items or [])}
    reverted_items = []
    for item in doc.items:
        if not is_locked_row(item):
            continue
        prior = before_by_name.get(item.name)
        if not prior:
            continue
        changed = False
        for fieldname in _PICK_LIST_LOCKED_FIELDS:
            if flt(item.get(fieldname)) != flt(prior.get(fieldname)) if fieldname == "qty" \
                    else item.get(fieldname) != prior.get(fieldname):
                item.set(fieldname, prior.get(fieldname))
                changed = True
        if changed:
            item.stock_qty = flt(item.qty) * flt(item.get("conversion_factor") or 1)
            reverted_items.append(item.item_code)

    if reverted_items:
        doc.calculate_taxes_and_totals()
        frappe.msgprint(
            _(
                "Qty/Item/Warehouse reverted for: {0}. These rows came from a "
                "submitted Pick List, which already reserved this exact "
                "amount — edit the Pick List instead if it needs to change."
            ).format(", ".join(reverted_items)),
            title=_("Locked to Pick List"),
            indicator="orange",
        )
...
def guard_so_fulfillment_route_lock(doc, method):
    """
    A Sales Order must ship consistently: once any line has gone out on a
    Delivery Note, every other line must also go through a Delivery Note —
    and once any line has gone out on a Sales Invoice with Update Stock, the
    rest must follow that route too. Mixing the two on one order leaves its
    delivery/billing reporting inconsistent. Enforced at before_submit (not
    validate) so this only blocks the actual submit action, never an
    ordinary draft save.
    """
    ...
    so_names = {getattr(item, field, None) for item in doc.items} - {None, ""}

    for so in so_names:
        if doc.doctype == "Delivery Note" and _so_has_submitted_stock_si(so):
            frappe.throw(_(
                "Sales Order {0} already has a Sales Invoice with Update Stock. "
                "This order is locked to the direct-billing route — a Delivery Note "
                "cannot be submitted for it."
            ).format(so))
        if doc.doctype == "Sales Invoice" and _so_has_submitted_dn(so):
            frappe.throw(_(
                "Sales Order {0} already has a submitted Delivery Note. "
                "This order is locked to the Delivery Note route — a Sales Invoice "
                "with Update Stock cannot be submitted for it."
            ).format(so))


@frappe.whitelist()
def create_sales_invoice_for_item_with_stock(sales_order, sales_order_item, item_code, qty):
    """
    (Called from the Sales Order stock widget's "Waiting for DN / SI" prompt)

    Mirrors create_pick_list_for_items' "map the whole order, then trim to
    the requested line" pattern: maps the standard Sales Order -> Sales
    Invoice mapper, keeps only the requested line, caps its qty at whatever
    was actually picked (never raises it), and forces Update Stock — the
    direct-billing alternative to Delivery Note for this one line.

    Returns an UNSAVED Sales Invoice for the user to review — nothing is
    written here.
    """
    from erpnext.selling.doctype.sales_order.sales_order import make_sales_invoice

    doc = make_sales_invoice(sales_order)
    qty = flt(qty)

    keep = []
    for row in doc.items:
        if row.so_detail == sales_order_item:
            if qty and qty < flt(row.qty):
                row.qty = qty
            keep.append(row)

    if not keep:
        frappe.throw(_(
            "Nothing pending to invoice for this item on {0}. It may already be "
            "delivered, invoiced, or not yet picked."
        ).format(sales_order))
...
def validate_quotation(doc, method=None):
    ...
    """
    # Prevent changing custom_lead_owner after submission
    if doc.docstatus == 1:
        doc_before = doc.get_doc_before_save()
        if doc_before and doc.custom_lead_owner != doc_before.custom_lead_owner:
            frappe.throw(_("Cannot change Lead Owner once Quotation is submitted."))
    ...
    if lead_name:
        ...
        if doc.docstatus == 0:
            ...
            if lead_owner and doc.get("custom_lead_owner") != lead_owner:
                frappe.db.set_value("Sales Order", doc.name, "custom_lead_owner", lead_owner, update_modified=False)
...
def _split_and_insert_pick_lists(merged_doc):
    """
    Insert one Pick List per Sales Order line from an already-built merged
    Pick List doc (e.g. from ERPNext's create_pick_list mapper). Never
    combines more than one line into a single Pick List. Pick
    List.before_save() re-derives allocation from live stock for whichever
    doc it's called on, and excludes stock already claimed by Pick Lists
    inserted earlier in this same loop, so sequential per-line inserts never
    double-allocate. Returns the list of inserted Pick List names.
    """
    header = merged_doc.as_dict()
    header.pop("name", None)
    header.pop("locations", None)

    by_line = {}
    for loc in merged_doc.locations:
        by_line.setdefault(loc.sales_order_item or loc.item_code, []).append(loc.as_dict())

    sales_order_name = (merged_doc.locations[0].sales_order
                         if merged_doc.locations and merged_doc.locations[0].get("sales_order") else "?")

    created = []
    for key, locs in by_line.items():
        try:
            pl = frappe.get_doc({**header, "doctype": "Pick List", "locations": locs})
            for idx, loc in enumerate(pl.locations, start=1):
                loc.idx = idx
            pl.insert(ignore_permissions=True)
            created.append(pl.name)
        except Exception:
            frappe.log_error(title=f"Pick List creation failed for {sales_order_name} / {key}",
                              message=frappe.get_traceback())
    return created


def _create_pick_lists_one_per_item(sales_order_name):
    """
    One Pick List per Sales Order line, instead of ERPNext's default single
    Pick List with every stocked line bundled into its child table — so each
    line can be picked, reviewed and submitted independently. Calls the
    standard mapper once (correct field mapping/allocation across the whole
    order), then hands off to _split_and_insert_pick_lists.
    """
    merged = create_pick_list(sales_order_name)
    if not merged.get("locations"):
        return []
    return _split_and_insert_pick_lists(merged)
...
def sales_order_on_submit(doc, method):
    ...
    try:
        _create_pick_lists_one_per_item(doc.name)
    except Exception:
        frappe.log_error(title=f"Pick List creation failed for {doc.name}", message=frappe.get_traceback())
...
def guard_po_item_not_over_so_need(doc, method=None):
    ...
    line must never total more than that line's own required qty, across
    EVERY Purchase Order raised against it — not just this one. Over-
    ordering here is exactly what forces the downstream defenses this app
    already has to carry (create_putaway_picklist's own allocation cap, the
    Item Stock & Action Plan widget's "shortfall vs already-on-order"
    reconciliation) — better to block it at the source than reconcile it
    after the fact every place it could resurface.

    Scoped strictly to rows that actually carry a `sales_order_item` link —
    a row for the very same item added to the very same PO WITHOUT that
    link (general stock, a different customer's need, anything not
    claiming to serve this SO line) is entirely unrestricted; it was never
    counted as fulfilling this SO's need in the first place, by any of the
    RM/coverage math elsewhere in this app (all of it keys off
    sales_order/sales_order_item, never a bare item_code match).
    """
    so_item_qty = {}
    for item in doc.items:
        ...
        if not so_item_id:
            continue
        row_qty = flt(item.get("fg_item_qty")) if doc.get("is_subcontracted") else flt(item.get("qty"))
        so_item_qty[so_item_id] = so_item_qty.get(so_item_id, 0) + row_qty

    if not so_item_qty:
        return

    for so_item_id, this_doc_qty in so_item_qty.items():
        so_item = frappe.db.get_value(
            "Sales Order Item", so_item_id, ["qty", "item_code", "parent"], as_dict=True
        )
        if not so_item:
            continue
...
@frappe.whitelist()
def fetch_multi_order_requirements(exclude_mr=None):
    # 1. Fetch pending Sales Order items
    so_items_raw = frappe.db.sql("""
        SELECT
            so.name as so_id, so.customer, so.customer_name,
    ...
    # "Demand" kept asking for the full shortfall even after a PO was
    # already placed for it. Two independent sourcing paths, same as the
    # Item Stock & Action Plan's own RM tracking
    # (get_item_stock_details_bulk): a direct-buy Purchase Order Item, or raw
    # material already allocated to a subcontracting PO via the custom
    # Purchase Order Raw Material Source table — both add, never replace
    # each other, and both are netted against what's already been received.
    ...
    # NOTE: a finished good already ordered on an existing subcontracted PO
    # is NOT, by itself, proof that nothing further needs sourcing for this
    # line — the resulting FG can still fall short of the line's full qty
    # (e.g. some of it sent on again for embroidery finishing), and "was a
    # PO raised" has no way to see that, only live stock does. val["shortage"]
    # below is already stock-based (val["available"], from bin_map, nets out
    # reservations and this order's own picks) so it already reflects
    # whatever a subcontract PO actually produced — no separate
    # "already subcontracted" carve-out needed on top of that.
    ...
    # Picked qty and pending-MR are computed in a second pass, once every
    # line belonging to each (SO, item, warehouse, bom_no) group is known.
    # The same item_code can appear twice on one Sales Order — once with a
    # BOM (built via production), once without (a plain stock line) — and
    # matching by item_code + warehouse alone would credit a Pick List or MR
    # raised for one line's shortage against the other, silently shrinking
    # (or hiding) the BOM line's real raw-material need. Scoping by the
    ...
    for val in aggregated_so_data.values():
        ...
        # factor too, so a BOM item partly covered by its own finished-good
    ...
    # consumed into a Subcontracting Receipt), company-wide, across every
    # Sales Order — purely informational (available/qty_needed above already
    # read live Bin stock, so final_shortage below is already correct); this
    # just explains WHY Stock is lower than a bulk purchase would otherwise
    # suggest, the same clarity gap flagged on the Item Stock & Action Plan's
    # own "Outstanding at Jobber" column. Once a transfer is consumed it has
    # already turned into that other order's finished-good stock — a
    # completed, unrelated transaction — so this reads Subcontracting Order
    # Supplied Item's own supplied/consumed/returned ledger, not just every
    # transfer ever made.
    rm_transferred_map = {}
    if rm_aggregation:
        rm_item_codes_for_transfer = tuple({k[0] for k in rm_aggregation.keys()}) or ("",)
        rm_transfer_rows = frappe.db.sql("""
            SELECT sosi.rm_item_code as item_code,
                   SUM(sosi.supplied_qty - sosi.consumed_qty - sosi.returned_qty) as outstanding_qty
            FROM `tabSubcontracting Order Supplied Item` sosi
            JOIN `tabSubcontracting Order` sco ON sco.name = sosi.parent AND sco.docstatus = 1
    ...
    for rm_key, rm in rm_aggregation.items():
        ...
        # bypassing MR) or pending MR, all cover part of the need before
...
@frappe.whitelist()
def download_quotation_pdf(quotation):
    if not quotation:
        frappe.throw(_("Quotation is required"))

    # Fetch document and verify user read permission
    doc = frappe.get_doc("Quotation", quotation)
    doc.check_permission("read")

    # Generate PDF directly using imported get_print
    pdf = get_print(
        doctype="Quotation",
        name=quotation,
        print_format="Quotation print format 7Apr",
        letterhead="DAC Letter Header quotation",
        no_letterhead=0,
        as_pdf=True
    )
...
def after_insert_quotation(doc, method):
    """
    Share the Quotation with custom_lead_owner if it's set on insertion.
    """
    if doc.custom_lead_owner:
        if not frappe.db.exists("DocShare", {"share_doctype": doc.doctype, "share_name": doc.name, "user": doc.custom_lead_owner}):
            from erp_dacsinc_custom.order_flow_api import add_docshare
            add_docshare(doc.doctype, doc.name, doc.custom_lead_owner, read=1, write=1, share=1, notify=0)


def on_update_quotation(doc, method):
    """
    Share the Quotation with custom_lead_owner if it changes in draft mode.
    """
    if doc.docstatus == 0:
        doc_before = doc.get_doc_before_save()
        old_owner = doc_before.custom_lead_owner if doc_before else None
        new_owner = doc.custom_lead_owner

        if new_owner and new_owner != old_owner:
            from erp_dacsinc_custom.order_flow_api import add_docshare
            if not frappe.db.exists("DocShare", {"share_doctype": doc.doctype, "share_name": doc.name, "user": new_owner}):
                add_docshare(doc.doctype, doc.name, new_owner, read=1, write=1, share=1, notify=0)
            
            if old_owner:
                # Downgrade old owner's access to read-only, matching Lead behavior
...
@frappe.whitelist()
def create_dn_or_si_from_pick_lists(sales_order, pick_lists, doctype):
    """
    Creates a draft Delivery Note or Sales Invoice from the specified Pick Lists
    linked to the Sales Order.
    """
    if isinstance(pick_lists, str):
        pick_lists = json.loads(pick_lists or "[]")
        
    if not pick_lists:
        frappe.throw(_("No Pick Lists selected."))

    # This order is already committed to one route or the other (see
    # guard_so_fulfillment_route_lock, enforced again at submit) — block it
    # here too, before a draft is even built, rather than letting the user
    # fill in a document that will only fail at the very end.
    if doctype == "Delivery Note" and _so_has_submitted_stock_si(sales_order):
        frappe.throw(_(
            "Sales Order {0} already has a Sales Invoice with Update Stock. "
            "This order is locked to the direct-billing route — a Delivery Note "
    ...
    if doctype == "Sales Invoice":
        ...
        if submitted_dns:
            ...
            # (standard billing), not directly from the Pick List.
            from erpnext.stock.doctype.delivery_note.delivery_note import make_sales_invoice as make_sales_invoice_from_dn
            dn_name = submitted_dns[0].name
            doc = make_sales_invoice_from_dn(dn_name)
            return doc.as_dict()

    # Get all the items and warehouses from the eligible Pick Lists.
    # Excludes Pick Lists already Completed — fully delivered/invoiced, so
    # mapping them again here would build a duplicate document. A Pick List
    # that's only "Partly Delivered" still has a remaining, un-delivered
    # balance on its line (picked_qty minus what's already gone out via an
    # earlier DN/SI — see custom_script._calculate_pick_list_delivery_status
    # and Pick List Item.delivered_qty, which ERPNext's own DN/SI submit
    # keeps in sync via the pick_list_item reference), so it belongs here
    # too — the SUM below nets that off so only the outstanding remainder is
    # mapped, never the already-shipped/invoiced portion again. Defense-in-
    # depth: the client already filters/nets these before offering this
    # action, but a stale client list must not be trusted blindly.
    # "Open" is Pick List's own core status for "nothing delivered yet" —
    # there is no "Submitted" option on this doctype.
    pick_items = frappe.db.sql("""
        SELECT
            pli.item_code, pli.warehouse,
            SUM(pli.picked_qty - IFNULL(pli.delivered_qty, 0)) AS qty,
            pli.sales_order_item, pli.parent AS pick_list, pli.name AS pick_list_item
        FROM `tabPick List Item` pli
        JOIN `tabPick List` pl ON pl.name = pli.parent
        WHERE pli.sales_order = %(so)s AND pl.name IN %(pls)s AND pl.docstatus = 1
          AND pl.status IN ('Open', 'Partly Delivered')
        GROUP BY pli.item_code, pli.warehouse, pli.sales_order_item, pli.parent, pli.name
        HAVING SUM(pli.picked_qty - IFNULL(pli.delivered_qty, 0)) > 0.0001
    """, {"so": sales_order, "pls": tuple(pick_lists)}, as_dict=True)
    ...
    if doctype == "Delivery Note":
        ...
        from erpnext.selling.doctype.sales_order.sales_order import make_sales_invoice
        doc = make_sales_invoice(sales_order)
        # This Sales Invoice is being built directly from Pick Lists, not a
        # Delivery Note — that's only a meaningful route when it actually
        # moves the stock. Always Update Stock here, regardless of
        # skip_delivery_note (which only affects the SO's own default
        # workflow expectation, not what this specific action does).
        doc.update_stock = 1

    # Map by sales_order_item first, then by item_code
    pick_by_so_item = {}
    pick_by_item_code = {}
    for p in pick_items:
        if p.sales_order_item:
            pick_by_so_item.setdefault(p.sales_order_item, []).append(p)
        else:
            pick_by_item_code.setdefault(p.item_code, []).append(p)

    keep_items = []
    for item in doc.items:
        so_item = item.so_detail or item.sales_order_item
        matches = []
        if so_item in pick_by_so_item:
            matches = pick_by_so_item[so_item]
        elif item.item_code in pick_by_item_code:
            matches = pick_by_item_code[item.item_code]
        ...
        if matches:
            ...
            for p in matches:
                if first:
                    item.qty = p.qty
                    item.warehouse = p.warehouse
                    item.stock_qty = p.qty * flt(item.conversion_factor or 1)
                    if doctype == "Delivery Note":
                        item.against_pick_list = p.pick_list
                        item.pick_list_item = p.pick_list_item
                    keep_items.append(item)
                    first = False
                else:
                    new_item = frappe.copy_doc(item)
                    new_item.qty = p.qty
                    new_item.warehouse = p.warehouse
                    new_item.stock_qty = p.qty * flt(new_item.conversion_factor or 1)
                    if doctype == "Delivery Note":
                        new_item.against_pick_list = p.pick_list
                        new_item.pick_list_item = p.pick_list_item
                    keep_items.append(new_item)

    doc.items = keep_items
    
    # Run standard validation/processing on client request by returning doc as dict
    return doc.as_dict()
